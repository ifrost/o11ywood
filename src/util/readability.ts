...
export var getReadabilityScores = function(text) {
  /*
  ...
  var lexiconCount = function(text, useCache, ignoreSample) {
    if (useCache && cache.lexiconCount) {
  ...
  var syllableCount = function(text, useCache) {
    if (useCache && cache.syllableCount) {
      return cache.syllableCount;
    }
    var count = 0;
  ...
  var sentenceCount = function(text, useCache) {
    if (useCache && cache.sentenceCount) {
  ...
  var fleschReadingEase = function(text) {
    ...
    return legacyRound(
      freBase.base - freBase.sentenceLength * sentenceLength - freBase.syllablesPerWord * syllablesPerWord,
      2
    );
  };
  ...
  var fleschKincaidGrade = function(text) {
    var sentenceLength = avgSentenceLength(text);
    var syllablesPerWord = avgSyllablesPerWord(text);
    return legacyRound(0.39 * sentenceLength + 11.8 * syllablesPerWord - 15.59, 2);
  };
  ...
  var automatedReadabilityIndex = function(text) {
    ...
    var readability = 4.71 * legacyRound(a, 2) + 0.5 * legacyRound(b, 2) - 21.43;
  ...
  (function() {
    ...
    var medianGrade = legacyRound(grade.length % 2 ? grade[midPoint] : (grade[midPoint - 1] + grade[midPoint]) / 2.0);
